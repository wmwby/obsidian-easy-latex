...
一个 Obsidian 插件，提供智能 LaTeX 数学公式自动补全，支持中英文关键词触发和 AI 驱动的公式生成。
...
在数学环境中直接输入中文数学术语（如 `积分`、`矩阵`、`极限`），无需反斜杠，插件会自动匹配对应的 LaTeX 命令。同时支持前缀子串匹配，例如输入 `约等于` 可以匹配到关键词 `约等` → `\approx`。
...
- **中文关键词**：直接输入触发。例如映射 `分数` → `\frac{$1}{$2}`，输入 `分数` 即可替换为 `\frac{}{}`。
